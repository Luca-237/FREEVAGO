...
// Corre el scraper de vuelos y el de hoteles para cada destino, arma UN
// ScrapingResult con todo junto y lo guarda. Si algún destino falla en
// vuelos u hoteles (sitio caído, sin resultados, etc.) no aborta todo el
// pedido: lo registra en "warnings" y sigue con el resto. Solo falla si al
// final no quedó NINGÚN resultado utilizable.
export async function generarYGuardarScraping({ destinos, origen, fechaIda, fechaVuelta, pasajeros, userSelectionId }) {
    ...
    try {
        const scrapingResult = await ScrapingResult.create({
            ...
            vuelos,
            hoteles,
            actividades: [], // todavía no hay scraper de actividades
