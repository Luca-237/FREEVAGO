...
// Ya no necesitamos kiwiApi: los IATA se obtienen directamente de las sugerencias de Turismocity
...
async function runOrchestration(params, page) {
    ...
    // 1. Usar los códigos IATA pre-resueltos desde las sugerencias de Turismocity
...
function startApiServer() {
    ...
    app.post('/api/viaje', async (req, res) => {
        ...
        try {
            if (!globalPage) await initBrowser();
            // Para el modo API, si no nos mandan IATA, los buscamos desde las sugerencias
            ...
            const params = {
                ...
                originIata: resolvedOriginIata,
                destinationIata: resolvedDestinationIata
            };
...
async function startConsoleMode() {
    ...
    try {
        ...
        const tcOriginName = sugOrigen[selectedIdxOri].displayName || sugOrigen[selectedIdxOri].cityName;
        const tcOriginIata = sugOrigen[selectedIdxOri].iata;
        ...
        const tcLocationName = sugerencias[selectedIdx].displayName || sugerencias[selectedIdx].cityName; 
        const tcDestinationSlug = sugerencias[selectedIdx].slug;
        const tcDestinationIata = sugerencias[selectedIdx].iata;
        ...
        const params = {
            ...
            originIata: tcOriginIata,
            destinationIata: tcDestinationIata
        };
